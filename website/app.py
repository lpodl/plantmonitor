...
def plot(df):
    ...
    ax.set_ylim(15, 35)
    ax.set_title(f"Temperature starting from {df['Timestamp'].iloc[0].strftime('%Y-%m-%d-%H-%m')})")
...
@app.route('/')
def index():
    today = "2024-08-08" # datetime.now().strftime('%Y-%m-%d')
    ...
    return render_template('index.html', temperature_plot=temperature_plot_data, humidity_plot=humidity_plot_data)


@app.route('/plant_photo')
def plant_photo():
    photo_path = '/home/justin/plantmon/pics/2024-08-08_1052.jpg'
